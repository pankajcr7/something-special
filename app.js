...
const langInstructions=isHinglish?`Language: HINGLISH — casual Hindi+English mix in Roman script like real young Indians text on WhatsApp.
HINGLISH RULES:
...
- Sound like real WhatsApp texts: "arre yaar tu toh full cute hai", "chal na coffee pe chalte h", "teri smile dekh ke dimag kharab 😭"
- NO Devanagari. NO formal aap/aapko — use tu/tujhe/tere
- BAD: "Aapki aankhein bahut sundar hain" → GOOD: "yaar teri eyes toh kamaal h 🔥"
- BAD: "Main aapko pasand karta hoon" → GOOD: "tu mujhe bohot acchi lagti hai ngl"`:isHindi?'Language: Casual Roman Hindi (NO Devanagari). Like real Indian WhatsApp texting. Use shortcuts: nhi, acha, thk h, chl. NO formal Hindi.':`Language: ${opts.lang}.`;
...
const messages=[{role:'system',content:sys},{role:'user',content:`Generate ${opts.count||5} ${opts.intensity} ${opts.style} ${opts.category} lines.${isHinglish?' Remember: Hinglish only — Hindi+English mix in Roman script like real Indian texting. NO pure English.':''}`}];
...
cap.Plugins.KeyboardSetup.openInputMethodSettings();
return;
}
}catch(e){}
showToast('Go to Settings → System → Languages & Input → Manage Keyboards');
}
...
const cap=window.Capacitor;
if(cap&&cap.Plugins&&cap.Plugins.KeyboardSetup){
cap.Plugins.KeyboardSetup.openInputMethodPicker();
return;
}
}catch(e){}
